...
// OAuth client id for the Codex "simplified" CLI flow. This is the same public
// client id the official Codex CLI uses to connect a ChatGPT subscription;
// override via ERGO_OPENAI_CLIENT_ID if you run your own OAuth app.
const CLIENT_ID =
  process.env.ERGO_OPENAI_CLIENT_ID ?? 'app_EMoamEEZ73f0CkXaXp7hrann';
const AUTHORIZE_URL =
  process.env.ERGO_OPENAI_AUTHORIZE_URL ??
  'https://auth.openai.com/oauth/authorize';
const TOKEN_URL =
  process.env.ERGO_OPENAI_TOKEN_URL ?? 'https://auth.openai.com/oauth/token';
...
export function describeOAuthHttpError(
  ...
  if (
    ...
    return `OAuth ${operation} rejected: the OAuth code or refresh token is invalid or expired. Re-run \`ergo auth login\`.`;
  }
  if (status === 400 && errCode === 'invalid_request') {
    return `OAuth ${operation} rejected: invalid request (${detail}).`;
  }
  if (status === 401 || status === 403) {
    return `OAuth ${operation} rejected by OpenAI auth (HTTP ${status}: ${detail}). Re-run \`ergo auth login\`.`;
  }
  if (status === 429) {
...
async function exchangeForApiKey(idToken: string): Promise<string | undefined> {
  ...
  try {
    ...
    // Network failure during the id_token swap is non-fatal: callers fall back
    // to the Codex backend with the access_token Bearer.
    return undefined;
  }
  if (!response.ok) {
    return undefined;
...
/**
 * Parse the OAuth callback the user can paste back into the terminal when the
 * localhost redirect doesn't work (firewalled / remote dev). Accepts a full URL
 * or a bare query string. Throws on missing code or state mismatch.
 */
  ...
  const credential = new Promise<CredentialRecord>(
    (resolvePromise, rejectPromise) => {
          ...
          if (candidate.length > 0) {
            try {
              ...
              process.stderr.write(
                `[ergo] ignoring pasted line: ${err instanceof Error ? err.message : String(err)}\n`,
              );
            }
          }
      ...
      const server = createServer((request, response) => {
        ...
        response.end(
          'Login complete. You can close this tab and return to ergo.',
        );
        void exchangeCode(code, redirectUri, verifier).then(
          settleResolve,
          settleReject,
      ...
      server.on('error', (err) => {
        process.stderr.write(
          `[ergo] OAuth callback server failed on port ${port}: ${err.message}\n` +
      ...
      try {
        ...
        process.stderr.write(
          `[ergo] stdin paste fallback unavailable: ${err instanceof Error ? err.message : String(err)}\n`,
        );
      }
    },
  );
...
export function createApiKeyCredential(
  apiKey: string,
  provider: CredentialRecord['provider'] = 'codex',
  baseUrl?: string,
): CredentialRecord {
  return {
    provider,
...
export async function refreshOAuthCredential(
  ...
  if (!current.refreshToken) {
    throw new Error(
      'OAuth credential has no refresh token; re-run `ergo auth login`',
    );
  }
